...
async function addEntry(name = '', status = "Completed", score = 0, startDate = new Date(), finishDate = null, rewatchCount = 0, note = '',currentEpisode=0) {
    try {
        ...
        if (typeof score !== 'number' || isNaN(score)) score = DEFAULT_SCORE; // Assuming DEFAULT_SCORE is defined elsewhere
        ...
        if (typeof rewatchCount !== 'number' || isNaN(rewatchCount)) rewatchCount = DEFAULT_REWATCH_COUNT; // Assuming DEFAULT_REWATCH_COUNT is defined elsewhere
        if (typeof note !== 'string') throw new Error('Note must be a string.');
        if (typeof currentEpisode !== 'number' || isNaN(currentEpisode)) currentEpisode = DEFAULT_CURRENT_EPISODE; // Assuming DEFAULT_CURRENT_EPISODE is defined elsewhere
